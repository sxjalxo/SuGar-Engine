...
```cpp
onClick() { inventoryWindow.hide(); }   // WRONG: mutates UI state in a callback
onClick() { emit(CloseScreen{...}); }   // RIGHT: intent -> system -> ECS
```
...
```
RuntimeUIView ──reads ECS──► updates RmlUi     [ok]
...
## The RmlUi effects compositor (M4 #14) — box-shadow, and the render-pass constraint

**Finding.** RmlUi 6 renders effects (`box-shadow`, `filter: blur`, mask images) through a
*layer* path on the `RenderInterface`: `PushLayer` / `PopLayer` / `CompositeLayers` /
`SaveLayerAsTexture` / `CompileFilter`. Our hand-written Vulkan backend implemented only core
geometry/texture/scissor, so box-shadow's internal white mask geometry leaked to screen and its
shadow texture resolved to the fallback white texture — it rendered as an **opaque white quad**.
RmlUi's *own* reference Vulkan backend never implemented these either (only the GL3 backend did),
so there was nothing to port; it was a from-scratch feature.

**The blocking constraint.** box-shadow's texture is generated *during* `Context::Render` (via a
`CallbackTexture`), and `PushLayer` must switch render targets — which Vulkan forbids inside an
open render pass. The runtime UI used to render appended to the **scene** render pass, so there
was no way to open an offscreen layer. **Resolution:** the UI now renders in its **own pass
after** the scene pass ends. The scene pass hands the image off in `COLOR_ATTACHMENT_OPTIMAL`;
the UI's layer-0 pass LOADs it (re-openable, because effect passes interleave), and `endFrame`
barriers it to `SHADER_READ_ONLY` for ImGui. With no effects present the output is
pixel-identical — verified before any effect code was added (Stage 1 of a staged build).

**What we built (the seam, Rule 22), and what we didn't (Rule 8).** Offscreen colour-layer pool
+ offscreen pass; `CompositeLayers` = a fullscreen triangle sampling one layer onto another
through a single-pass Gaussian blur (`rml_composite.frag`) or a copy; `SaveLayerAsTexture` copies
the layer's *scissor region* into a **persistent** image, because RmlUi caches shadow textures
across frames while the layer pool is transient. **Stencil clip masks are a deliberate no-op**:
they only matter for a shadow under a *translucent* element; opaque elements (all our HUD chips)
cover the shadow's centre anyway. A forced-later gap, documented, not silent.

**Bug caught by instrumentation, not guessing.** The first offscreen attempt still showed dark
boxes. A one-shot `fprintf` in the draw loop proved the pipelines/plumbing were correct — the
real cause was the *blended* pipelines writing the sprite's cutout alpha into the ImGui-composited
image, punching holes. Fix: blended pipelines preserve destination alpha. (Same class of bug as
#13; the offscreen image ImGui composites is unforgiving about alpha.)

Design record: `ROADMAP.md` friction #14. Files: `RmlVulkanRenderer.{h,cpp}`,
`Renderer::createUiLayerPass`, `shaders/rml_fullscreen.vert` + `rml_composite.frag`.
