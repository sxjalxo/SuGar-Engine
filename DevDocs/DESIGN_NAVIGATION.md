...
3. **Off-mesh links** (jumps, ladders, doors) — an edge that is not a shared polygon
   ...
   status. **BUILT — see the addendum at the end of this record (M4 L3).**
...
## Addendum (M4 L3, 2026-08-11) — off-mesh links

### What forced it

The voxel game's mobs. A navmesh welds by vertex, so two walkable surfaces that do not
share corners are separate islands however close they are: a mob standing below a
two-block ledge, or on the far side of a one-block trench, plans and gets back
`Unreachable` — correctly, because on the mesh alone there is no route. The first
workaround (a smoothed heightfield, so a one-block step becomes a 45° ramp) covers steps
and nothing else. Chasing a player who jumped down a cliff needs a connection the surface
geometry does not have.

### The decision

A **link is mesh data, not a second graph**: `NavMesh::links`, a vector of

```cpp
struct NavLink {
    glm::vec3 start, end;      // authored world-space endpoints
    float cost = 0.0f;         // 0 = charge the 3D distance
    bool bidirectional = true;
    int startPolygon, endPolygon;   // DERIVED — resolved by buildAdjacency
};
```

- **The endpoints are authored; the polygons are derived.** `buildAdjacency()` already
  recomputes `neighbors` from geometry precisely so an asset cannot carry a stale copy;
  link resolution rides on the same call for the same reason. A rebaked mesh re-resolves
  its links and nothing dangles.
- **The search learns one new edge kind, not a new search.** A* expands a polygon's shared
  edges as before, then its outgoing links: entering across a link costs
  `distance(entryPoint, link.start) + linkCost`, and the entry point on the far side is
  `link.end`. Determinism is unchanged — links are expanded in index order after edges,
  and the frontier's total order `(f, polygon)` is untouched.
- **String-pulling splits at links.** The funnel is only valid across shared portals, so a
  corridor that uses links is funnelled in *runs*: each contiguous polygon run is
  string-pulled on its own, and the link's two endpoints are emitted as waypoints between
  runs. That is the standard treatment (Detour does the same with off-mesh connections),
  and it means the agent walks to the link's start, then to its end — no special steering
  and no new agent state.
- **No new component, no new status.** `NavAgentComponent` still holds a list of
  waypoints; two of them happen to be a link's endpoints. A jump *animation* would need to
  know which waypoint pair is a link — that is the next thing a game can force, and it is
  additive (the corridor already records which step used which link).

### Deliberately not built

Link areas/flags (only some agents may use a ladder), traversal time/animation hooks,
auto-generated links from geometry (the *game* knows a one-block gap is jumpable; the
engine would have to guess), and dynamic enable/disable at runtime. Each waits for a game.


## Addendum — what a streaming game did to this design (2026-08-13)

The voxel dogfood re-bakes the navmesh on every chunk boundary and on every burst of player
edits, which turned three things that were fine for an authored level into measured problems.
All three fixes kept the public shape of navigation unchanged.

**Point queries were linear scans.** `findContainingPolygon` and `findNearestPolygon` walked
every polygon. That is invisible in a hand-authored level and 24 ms per bake at 18 000
polygons — and the same scan runs twice per *path request* (start and destination snapping),
so the cost was on gameplay too. `NavMesh::lookupGrid` is an XZ uniform grid of polygon
indices, **derived** by `buildAdjacency` beside the neighbour table so it cannot go stale.
Containment reads one cell; nearest walks expanding rings and stops only when the closest
possible point of the next ring cannot beat the best found. Candidates are visited in
ascending polygon index, so the documented lowest-index tie-break is unchanged — and
`NavLookupGrid` asserts *equivalence* against the still-compiled scan over 400 sampled points
rather than merely asserting plausible answers.

**Welding dominated the bake.** The vertex welder's spatial hash used cells of exactly
`weldEpsilon`, which forces a 3x3x3 probe for correctness (a corner can weld with one two
cells away). Cells of **2·epsilon** make a 2x2x2 probe provably sufficient: per axis, only one
neighbour can hold a point within epsilon, and both directions cannot hold at once. Weld cost
9.57 -> 3.37 ms. `NavWeldProbe` puts corner pairs *exactly on* cell boundaries, which is the
case a too-small probe silently misses.

**Re-issuing a destination threw the path away.** `setDestination` reset `status` to Idle
unconditionally, so the ordinary chase pattern — call it every tick at the thing you are
chasing — meant a full A\* per agent per frame. 58 agents took a 243 FPS scene to 0.89. It is
now a no-op when the target is unchanged *and* the agent is already Following; Unreachable and
Arrived still re-arm, so the documented "re-issue to retry" behaviour survives.

### Still open, with numbers attached

- **Tiled navmesh / dirty-tile rebuild.** A one-block edit costs a whole bake (11.9 ms at
  18 000 polygons). The game debounces around it — one bake 0.25 s after the last edit of a
  burst — which is enough for a voxel game and would not be for one that changes geometry
  constantly. The shape when it is forced is Recast/Detour's: bake per tile, stitch at
  borders, rebuild only what changed.
- **Failed-replan backoff.** There is no wait between *failed* plans, so N agents chasing
  something behind a closed door cost N full searches per frame. This is deliberately not a
  constant someone picks: how long to wait, whether a newly opened route should wake an agent
  immediately, whether moving obstacles invalidate a failure, and whether the policy is
  engine-owned or gameplay-owned are all real questions. It wants a second game's evidence.
- **Off-mesh links are validated but unexercised.** A deliberately constructed island proves
  the whole chain (Unreachable -> link -> Success -> the agent actually crosses). A census of
  120 ordinary routes over natural terrain crosses **zero** links. That is a fact about world
  topology, not about the feature.
