...
1. **UI callbacks emit intents only. They never mutate persistent UI state
   ...
   ```
   onClick() {
       inventoryWindow.hide();      // WRONG: hides via widget state
   }
   ```
   ...
   ```
   onClick() {
       emit(CloseScreen{ Inventory }); // RIGHT: intent → system → UIScreenComponent
   }
   ```
...
2. **RmlUi does not subscribe to ECS. It is not event-driven over the model.** The
   ...
   ```
   RuntimeUISystem ──reads ECS──► updates RmlUi        [ok]
   Health changed ──► RmlUi callback fires              [no]
   ```
...
## Rendering: the effects compositor (added M4 #14)

Runtime UI is drawn through a hand-written `Rml::RenderInterface` (`RmlVulkanRenderer`) that
SuGar owns; RmlUi never touches the device, swapchain, or frame loop. Two properties matter:

1. **The UI renders in its own pass, after the scene pass.** RmlUi generates effect textures
   (`box-shadow`, `blur`) *during* `Context::Render` by pushing offscreen layers, and switching
   render targets is illegal inside an open render pass. So the UI is not appended to the scene
   pass any more: the scene pass leaves its image in `COLOR_ATTACHMENT_OPTIMAL`, the UI's
   layer-0 pass LOADs it (re-openable across interleaved effect passes), and `endFrame` barriers
   it to `SHADER_READ_ONLY` for the ImGui viewport. **With no effects the pixels are identical**
   to the old inline path.

2. **The compositor implements RmlUi 6's layer path as SuGar-owned Vulkan.** Offscreen colour
   layers (`PushLayer`/`PopLayer`), a fullscreen composite/blur (`CompositeLayers` +
   `CompileFilter("blur")`, single-pass Gaussian), and a persistent copy of a layer's scissor
   region (`SaveLayerAsTexture`, because RmlUi caches shadow textures across frames). Built as a
   **seam** (Rule 22) so a future effect is an extension, not a rewrite; scoped to what a game
   forced (Rule 8) — **stencil clip masks are a documented not-yet-built gap** (they only affect
   shadows under *translucent* elements; opaque elements hide the shadow's centre).

Full rationale + the interleaving/alpha bugs found: `docs/RUNTIME_UI_LESSONS.md`, `ROADMAP.md`
friction #14.

---

## Addendum (M4 L3, 2026-08-11) — world-space labels

### What forced it

Mob nameplates. The L3 game has 60+ creatures of a dozen kinds and 15 villager professions,
and no way to say which is which: the HUD can only describe what the crosshair is on, so
identifying anything means aiming at it one at a time. Every game with mobs, NPCs, damage
numbers or interaction hints needs text **attached to a point in the world**.

### The decision — it is UI anchored to a world point, not 3D text

Two shapes were available:

1. **Glyph geometry in the scene** — build quads from a font atlas and draw them in the
   scene pass. Real 3D text (rotatable, depth-tested), and it forks everything: a second
   font pipeline beside FreeType/RmlUi, an atlas asset, a shader, and its own layout code.
2. **Screen-space UI positioned from a world point** — project the anchor with the camera
   the pass already has, and let the existing RmlUi stack draw the text.

Option 2, and not only because it is smaller: the *content* of a nameplate is UI (it wants
the document's font, colour, and RCSS styling, and it must stay legible rather than
perspective-correct). This is what Unity's Screen-Space-Camera canvas and Unreal's widget
components do for the same reason. Option 1 becomes right when text has to live *in* the
scene — painted on a sign, lying on the floor — which no game here has asked for.

### Ownership

```
WorldLabelComponent{ text, offsetY, maxDistance }   ← authoritative, ECS, serialized
screen position, scale, visibility                  ← DERIVED, recomputed every frame
```

The anchor is the entity's own transform: no second position to desync. The projection is
a pure function of (view-projection, anchor, viewport), so it lives in Core as
`ScreenProjection::project` — testable headlessly, which is what keeps the one piece of
real math here from being renderer-only folklore.

### How it renders

The renderer projects each labelled entity, drops the ones behind the camera or past
`maxDistance`, keeps the **nearest N** (32) and hands the view a flat list. The view keeps a
fixed pool of `<span class="worldlabel">` elements inside a `#worldlabels` container — text
and inline `left`/`top`/`opacity` per frame, no markup rebuilt, and the document owns how a
label *looks* exactly as it owns the rest of the HUD. A document without that container
simply gets no labels.

### Deliberately not built

Depth occlusion (a label shows through a wall — it is screen-space UI, and the fix is a
depth query the renderer does not expose yet), per-label fonts/colours from the game (use a
class), rich text, and world-space rotation. Each waits for a game.
