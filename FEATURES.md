...
| `SUGAR_PROFILE=1` | per-system fixed-step timing to **stderr** once a second: median/max ms for each named system, the step total measured independently, and the residual. Collection is always on (~0.16 ms/step, measured); the editor Systems panel shows the same numbers live. Two further lines under the same knob: `[profile-render]` (`drawList`/`resources`/`fenceWait`/`record`/`submit`/`frameOther` vs an independent `frameTotal`) and `[profile-loop]` (`input`/`fileWatch`/`moduleCheck`/`fixedStep`/`sleep` vs an independent `loopTotal`, plus **`steps/frame`** — read it before summing any per-step figure with a per-frame one), and `[profile-drawlist]` (`reset`/`gather`/`items`/`skinning`/`sort`/`lights` vs an independent `buildTotal`, plus `skinnedItems`/`joints` and the work counts `subtreeSearches`/`nodeVisits`/`matrixHops`) |
